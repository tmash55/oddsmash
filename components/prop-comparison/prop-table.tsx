...
export function PropComparisonTable({
  ...
  const handleStatTypeChange = (newStatType: string) => {
    console.log("handleStatTypeChange called with:", newStatType);
    ...
    if (onPropTypeChange) {
      ...
      if (market) {
        ...
        console.log("Calling onPropTypeChange with:", urlPropType);
        
        // Call the parent's onPropTypeChange with just the prop type
        // (the parent will handle preserving the game parameter)
        onPropTypeChange(urlPropType);
        
        // Also directly update the URL to ensure it changes
        if (typeof window !== 'undefined') {
          // Get the current URL and extract the sport and game parameter
          const url = new URL(window.location.href);
          const pathParts = url.pathname.split('/');
          const sportParam = pathParts[1]; // e.g., "nba"
          
          // Preserve the game parameter if it exists
          const gameParam = url.searchParams.get('game');
          const searchParams = gameParam ? `?game=${gameParam}` : '';
          
          // Create the new URL and update it
          const newUrl = `/${sportParam}/props/${urlPropType}${searchParams}`;
          console.log("Directly updating URL to:", newUrl);
  ...
  // Initialize selectedEventId from URL query parameters
  const [selectedEventId, setSelectedEventId] = useState<string | null>(() => {
    if (typeof window !== 'undefined') {
      const urlParams = new URLSearchParams(window.location.search);
      const gameParam = urlParams.get('game');
      
      // If we have a short ID (likely from URL), try to find the full ID
      if (gameParam && gameParam.length <= 5) {
        return gameParam; // Will be resolved to full ID when events are loaded
      }
      return gameParam;
    }
    return null;
  ...
  // Track all available events for mapping short IDs to full IDs
  const [events, setEvents] = useState<Event[]>([]);
  
  // Fetch available events when sport changes
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        // Check if we have a shortened ID that needs resolving
        if (selectedEventId && selectedEventId.length <= 5) {
          const matchingEvent = sortedEvents.find(event => event.id.endsWith(selectedEventId));
          // We don't update the selectedEventId state here to avoid an infinite loop
          // since we are already handling shortened IDs in the fetchPlayerProps function
        }
      } catch (err) {
    ...
    fetchEvents();
  }, [sport]); // Only re-fetch when sport changes
  
  // Function to update the URL with the selected game ID (shortened to last 5 digits)
  const updateUrlWithGameId = (gameId: string) => {
    if (typeof window !== 'undefined') {
      // Get current URL parts
      const url = new URL(window.location.href);
      
      // Set the game parameter, using only the last 5 digits
      if (gameId) {
        const shortId = gameId.slice(-5);
        url.searchParams.set('game', shortId);
      } else {
  ...
  // Custom handler for game selection that updates both state and URL
  const handleGameSelect = (gameId: string) => {
    // We store the full ID in state, but use shortened ID in URL
    setSelectedEventId(gameId);
    updateUrlWithGameId(gameId);
    // The props will be fetched via the useEffect that watches selectedEventId
  };
  ...
  const fetchPlayerProps = async (refresh = false) => {
    ...
    // Check if we need to resolve a short ID to a full ID
    let fullEventId = selectedEventId;
    
    // Check if this is a shortened ID (5 or fewer characters)
    if (selectedEventId.length <= 5 && events.length > 0) {
      // Try to find the full ID by matching the end of the ID
      const matchingEvent = events.find(event => event.id.endsWith(selectedEventId));
      if (matchingEvent) {
        fullEventId = matchingEvent.id;
        // Note: we don't update selectedEventId here to avoid triggering an infinite loop
      } else {
        console.error("Could not find matching event for shortened ID:", selectedEventId);
        return;
      }
    }
    ...
    try {
      ...
      if (shouldFetchAlternate) {
        ...
        const response = await fetch(
          `/api/events/${fullEventId}/props?sport=${sport}&markets=${markets}&bookmakers=${selectedSportsbooks.join(
            ","
          )}`
        );
        ...
        const response = await fetch(
          `/api/events/${fullEventId}/props?sport=${sport}&markets=${standardMarket}&bookmakers=${selectedSportsbooks.join(
            ","
          )}`
        );
