...
export function SportsbookSelector() {
  ...
  const { preferences, isLoading, updatePreferences, getActiveSportsbooks } = usePreferences();
  const selectedSportsbooks = preferences?.preferred_sportsbooks || [];
  const userState = preferences?.state_code || null;
    ...
    if (hasChanges) {
      ...
      try {
        ...
        if (sportsbooksChanged) {
          await updatePreferences({ preferred_sportsbooks: tempSelections });
        }

        // Update the state if changed
        if (stateChanged) {
          await updatePreferences({ state_code: tempState });
        }
