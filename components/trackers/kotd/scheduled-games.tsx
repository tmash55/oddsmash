...
export default function ScheduledGames({ games }: ScheduledGamesProps) {
  ...
  const isGameEligible = (startTime: string) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {games.map((game) => {
        const isEligible = isGameEligible(game.startTime);
        return (
          <Card key={game.gamePk} className={`overflow-hidden ${isEligible ? 'border-yellow-200 dark:border-yellow-800' : ''}`}>
              ...
              <div className="p-4">
                ...
                <div className="flex justify-between items-center mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    {isEligible && (
                      <Badge variant="outline" className="bg-yellow-50 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400 border-yellow-200 dark:border-yellow-800">
                        KOTD Eligible
                      </Badge>
                    )}
                  </div>
