...
import type { OddsScreenItem, TableInteractionHandlers, OddsPrice } from '@/components/odds/types/odds-screen-types'
import { getSportsbookById, getAllActiveSportsbooks, type SportsbookMeta } from '@/data/sportsbooks'
...
const calculateUserBestOdds = (
  ...
  if (availableBooks.length > 0) {
    ...
    userBest = {
      ...
      link: bestUserBook.odds.link // Use first link for clickthrough
    }
  }
  ...
  if (allAvailableBooks.length > 0) {
    ...
    globalBest = {
      ...
      line: bestLine,
      book: allBestBooks.map(b => b.bookId).join(', '), // Join all book IDs
      link: bestGlobalBook.odds.link // Use first link for clickthrough
    }
  }
...
const renderAlternateRow = (
  ...
  const rowBg = altIndex % 2 === 0 ? 'bg-gray-100 dark:bg-gray-800' : 'bg-gray-200 dark:bg-gray-900'
                                  ...
                                  {ids.map((id, i) => {
                                    ...
                                    return sb?.image?.light ? (
                                      <div key={`${id}-${i}`} className="flex items-center gap-1">
                                        ...
                                        <span className="text-[11px] text-gray-500 dark:text-gray-400">{sb.name}</span>
                                      </div>
                                    ) : null
                                  })}
                                </div>
            ...
            >
              <div className="space-y-1">
                ...
                {(() => {
                  ...
                  if (avgPrice === null) {
                    return <div className="text-xs text-gray-400 dark:text-gray-500">-</div>
                  }
    ...
    {orderedSportsbooks.map((book) => {
      ...
      const renderAltOdds = (
        ...
        side?: 'over' | 'under'
          ...
          if (!priceOdds || !side) {
            return (
              <div className="block w-full text-xs text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md px-2 py-1.5 mx-auto text-center min-w-fit">
                -
              </div>
            )
          }
...
export function OddsTable({
  ...
  const updatePrimaryRowWithLine = useCallback(async (item: OddsTableItem, selectedLine: number) => {
    ...
    if (alternateData) {
      // Clear all cache entries for this row (with and without custom lines)
      const keysToDelete = Array.from(processedRowCache.keys()).filter(key => key.startsWith(rowKey))
      // Clear cache efficiently
      keysToDelete.forEach(key => processedRowCache.delete(key))
      
      // Store the custom line selection
  ...
  const resetToPrimaryLine = useCallback((item: OddsTableItem) => {
    ...
    // Clear all cache entries for this row
    const keysToDelete = Array.from(processedRowCache.keys()).filter(key => key.startsWith(rowKey))
    ...
    return [...filteredData].sort((a, b) => {
      ...
      switch (sortField) {
        case 'entity':
          ...
          break
        case 'event':
          aValue = `${a.event?.awayTeam} @ ${a.event?.homeTeam}`
          bValue = `${b.event?.awayTeam} @ ${b.event?.homeTeam}`
          break
        case 'bestOver':
        ...
        case 'bestUnder':
          ...
          break
        case 'startTime':
          aValue = new Date(a.event?.startTime || 0).getTime()
          bValue = new Date(b.event?.startTime || 0).getTime()
          break
        default:
          return 0
      }
  ...
  const displaySportsbooks = useMemo(() => {
    ...
    if (visibleSportsbooks && visibleSportsbooks.length > 0) {
      const filteredBooks = visibleSportsbooks
        ...
        .filter((book): book is SportsbookMeta => book !== undefined && book.isActive)
  ...
  const renderAverageOddsButton = (rowItem: OddsTableItem, side: 'over' | 'under', visibleSportsbooks: string[]) => {
    ...
    if (!displayOdds) {
      return (
        <div className="px-2 py-1.5 text-xs text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md text-center min-w-fit mx-auto">
          -
        </div>
      )
    }
  ...
  const renderBestOddsButton = (rowItem: OddsTableItem, side: 'over' | 'under', visibleSportsbooks: string[]) => {
          ...
          <TooltipContent>
            <div className="text-xs">
              ...
              <div className="text-gray-300 text-[10px] mt-1">
                at {getSportsbookById(globalBest.book)?.name || globalBest.book}
              </div>
            </div>
          </TooltipContent>
        </Tooltip>
    ...
    return (
      <div className="flex items-center gap-1">
        ...
        >
          <span className="font-medium text-gray-700 dark:text-gray-300 hover:text-blue-700 dark:hover:text-blue-300 transition-colors">
  ...
  if (!groupedData || groupedData.length === 0) {
    return (
      <div className={`bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden ${className}`}>
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            <div className="text-gray-900 dark:text-gray-100 font-medium mb-2">No Data Available</div>
                      ...
                      case 'entity':
                        return (
                          <SortableHeader
                            key="entity"
                            id="entity"
                          className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors sticky left-0 bg-gray-50 dark:bg-gray-900 z-10 border-r border-gray-200 dark:border-gray-700"
                      ...
                      case 'event':
                        return (
                          <SortableHeader
                            key="event"
                            id="event"
                            className="px-3 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors border-r border-gray-200 dark:border-gray-700"
                      ...
                      case 'best-line':
                        return (
                          <SortableHeader
                            key="best-line"
                            id="best-line"
                            className="px-3 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors border-r border-gray-200 dark:border-gray-700"
                            onClick={() => handleSort('bestOver')}
                          >
                            <div className="flex items-center justify-center">
                              <span>Best Line</span>
                              <SortIcon field="bestOver" />
                            </div>
                          </SortableHeader>
                        )
                      case 'average-line':
                ...
                >
                  {orderedSportsbooks.map((sportsbook) => (
                    <SortableHeader
                      key={sportsbook.id}
                      id={sportsbook.id}
                      className="py-3 pl-4 pr-2 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider min-w-[120px] border-r border-gray-200 dark:border-gray-700"
                      onClick={() => {
                    ...
                    >
                      <div className="flex flex-col items-center space-y-1 cursor-pointer hover:opacity-80 transition-opacity">
                        ...
                        />
                        <span className="text-[10px] text-gray-400 dark:text-gray-500">{sportsbook.name}</span>
                      </div>
                    </SortableHeader>
                  ))}
                </SortableContext>
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {groupedData.map((group, groupIndex) => {
              if (group.type === 'separator') {
                ...
                return (
                  <tr key={`separator-${group.date}`} className="bg-gray-600 dark:bg-gray-700">
                    {/* Left section */}
                    <td 
                      colSpan={columnsPerSection + (remainder > 0 ? 1 : 0)} 
                      className="px-4 py-1.5 text-sm font-semibold text-white dark:text-gray-200 border-t border-b border-gray-500 dark:border-gray-600 text-center"
                    >
                      {group.dateLabel}
                    </td>
                    {/* Middle section */}
               ...
               return (
                 ...
                 {filteredColumnOrder.map((columnId) => {
                   switch (columnId) {
                    case 'entity':
                      return (
                        <td key="entity" className={`px-4 py-4 whitespace-nowrap sticky left-0 ${rowBgClass} hover:bg-gray-50 dark:hover:bg-gray-700 z-10 border-r border-gray-200 dark:border-gray-700`}>
                          {item.entity?.type === 'game' ? (
                             <div>
                              ...
                              {/* Line Selector and Expand Controls for Game Props - Hidden for Moneyline */}
                     ...
                     case 'event':
                       return (
                         <td key="event" className="px-3 py-4 whitespace-nowrap border-r border-gray-200 dark:border-gray-700">
                     ...
                     case 'best-line':
                       return (
                        <td key="best-line" className="px-3 py-4 whitespace-nowrap text-center border-r border-gray-200 dark:border-gray-700">
                           <div className="space-y-1">
                     ...
                     case 'average-line':
                       return (
                        <td key="average-line" className="px-3 py-4 whitespace-nowrap text-center border-r border-gray-200 dark:border-gray-700">
                           <div className="space-y-1">
                 ...
                 {orderedSportsbooks.map((sportsbook) => {
                  ...
                  const renderOddsButton = (
                     odds: { line: number; price: number; link?: string },
                     side: 'over' | 'under'
                   ) => {
                     ...
                     return (
                       <Tooltip>
                         <TooltipTrigger asChild>
                           ...
                           >
                         <div className="text-center">
                           {/* For moneylines only, keep stacked format. All other markets use horizontal layout */}
                           {item.entity.type === 'game' && item.entity.details === 'Moneyline' ? (
                             /* Moneylines: stacked format (team over odds) */
                             <>
                               <div className="text-sm font-semibold">
                                 {formatOdds(odds.price)}
                               </div>
                               <div className="text-xs opacity-75">
                                 {formatLine(odds.line, side, item)}
                               </div>
                             </>
                           ) : (
                             /* All other markets: horizontal layout (line + odds) */
                             <div className="text-xs font-medium">
                               <span className="opacity-75">{formatLine(odds.line, side, item)}</span>
                               <span className="ml-1 font-semibold">{formatOdds(odds.price)}</span>
                             </div>
                           )}
                         </div>
                           </button>
                         ...
                         <TooltipContent>
                           <p>Place bet on {sportsbook.name}</p>
                         </TooltipContent>
                       </Tooltip>
                     )
                   }
                  ...
                  const renderPlaceholder = () => (
                   ...
                   return (
                      <td key={sportsbook.id} className="border-r border-gray-200 dark:border-gray-700 px-1 py-2 w-auto whitespace-nowrap">
                        <div className="space-y-1">
                    ...
                    <React.Fragment>
                      {isAltLoading && (
                        <tr className={index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-900'}>
                          <td colSpan={columnOrder.length + orderedSportsbooks.length} className="px-4 py-3 text-xs text-gray-500 dark:text-gray-400">
