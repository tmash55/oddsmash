...
export default function HighHitRate({ data, onParamsChange, params }: HighHitRateProps) {
  ...
  const handleShare = (player: HitConsistencyCandidate) => {
    ...
    const shareText = `⚾️ ${player.out_full_name} (${player.out_team_abbreviation}) - High Hit Rate\n` +
  ...
  return (
    <div className="space-y-6">
      ...
      <QuickHitTable
        ...
        columns={columns}
        title="High Hit Rate"
        subtitle="Players consistently hitting over their standard line"
        onSort={handleSort}
        sortField={sortField}
