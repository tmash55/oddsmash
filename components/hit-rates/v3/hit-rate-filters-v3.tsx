...
import { SupportedSport, SportMarket, SPORT_CONFIGS } from "@/types/sports"
...
interface HitRateFiltersV3Props {
...
export default function HitRateFiltersV3({
  sport,
  onMarketChange,
  onViewModeChange,
  onSearchChange,
...
}: HitRateFiltersV3Props) {
  const [isSearchExpanded, setIsSearchExpanded] = useState(false)
  const searchInputRef = useRef<HTMLInputElement>(null)
  const scrollContainerRef = useRef<HTMLDivElement>(null)
  const [isMobileFiltersOpen, setIsMobileFiltersOpen] = useState(false)

  // Get available markets for the current sport
  const sportConfig = SPORT_CONFIGS[sport]
  const availableMarkets = sportConfig.markets
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="outline" className="w-full justify-between">
                        <span>{availableMarkets.find((m) => m.value === currentMarket)?.label || currentMarket}</span>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="gap-1 h-8">
                  ...
                  <span className="font-medium">{availableMarkets.find((m) => m.value === currentMarket)?.label || currentMarket}</span>
                  <ChevronDown className="h-3.5 w-3.5 text-muted-foreground ml-1" />
                </Button>
              </DropdownMenuTrigger>
