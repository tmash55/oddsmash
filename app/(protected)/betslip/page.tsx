...
export default function BetslipPageRefactored() {
  ...
  const sortedBetslips = [...betslips].sort((a, b) => {
  ...
  const handleCompareOdds = async (betslipId: string) => {
    ...
    try {
      const response = await fetch('/api/betslip/compare-odds', {
        method: "POST",
      ...
      if (data.success) {
        const { finalizedBetslip, totalSelections, eventsRefreshed, totalMarkets } = data.data
        
        toast.success(
          `Betslip finalized! ${totalSelections} selections with fresh odds from ${eventsRefreshed} events (${totalMarkets} markets).`
        )
        
        // Navigate to the finalization review page
        router.push(`/betslip/${finalizedBetslip.id}`)
      } else {
