...
import { ROWS_FORMAT, type ArbRow } from "@/lib/arbs-schema";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const limit = Math.max(1, Math.min(1000, parseIntSafe(sp.get("limit"), 100)));
    ...
    const body: Record<string, any> = { format: ROWS_FORMAT, v: serverV, mode, ids, rows };
