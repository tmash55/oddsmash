...
export function PreferencesProvider({ children }: { children: React.ReactNode }) {
  ...
  const updatePreferences = useCallback(async (
    ...
    try {
      ...
      // Save to database
      const updatedPrefs = await preferencesRPC.updatePreferences(user.id, updates);
  ...
  const batchUpdate = useCallback(async (
    ...
    try {
      // Optimistic update
      const optimisticUpdates = updates.reduce((acc, { key, value }) => {
        acc[key] = value;
        return acc;
      ...
      // Save to database
      const updatedPrefs = await preferencesRPC.batchUpdatePreferences(user.id, updates);
      setPreferences(updatedPrefs);
    } catch (err) {
  ...
  const getArbitrageFilters = useCallback(() => {
    ...
    return {
      ...
      totalBetAmount: preferences.arbitrage_total_bet_amount ?? 200,
