...
import { fetchArbs, fetchRows, ArbRow } from "@/lib/arbs-client";
...
export function useArbsStream({ pro, live, eventId, limit = 100 }: { pro: boolean; live: boolean; eventId?: string; limit?: number }) {
  ...
  const loadPage = useCallback(async (opts?: { reset?: boolean }) => {
    const useCur = opts?.reset ? 0 : cursor;
    setLoading(true);
    try {
      ...
      if (!("unchanged" in res)) {
        ...
        setIds(res.ids);
        setHasMore(res.ids.length === limit);
        const prev = prevRef.current;
  ...
  return { rows, ids, changes, added, version, loading, connected, lastDiff, error, cursor, hasMore, nextPage, prevPage, setCursor, lastUpdated, refresh, authExpired, reconnectNow };
}
