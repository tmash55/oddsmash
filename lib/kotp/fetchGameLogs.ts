...
export const fetchAndCachePlayoffGameLogs = async () => {
  const url =
    "https://stats.nba.com/stats/leaguegamelog?Counter=0&DateFrom=&DateTo=&Direction=DESC&LeagueID=00&PlayerOrTeam=P&Season=2024-25&SeasonType=Playoffs&Sorter=DATE";
  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch playoff game logs: ${response.status}`
        );
      }
    ...
    if (error instanceof Error && error.name === "AbortError") {
      ...
      const staleLogs = (await redis.get(GAMELOG_CACHE_KEY)) as
        | PlayerGameLog[]
        | null;
      if (staleLogs) {
      ...
      console.log(
        "No stale data available, returning empty array since playoffs may not have started yet"
      );
      return [];
    }
